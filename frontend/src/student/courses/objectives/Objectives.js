...
const Objectives = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          props.getAdminTopicObjectives(props.data.id, token);
      }
    ...
    const saveProduct = (e) => {
        ...
        if (record.name.trim()) {
            ...
            let _record = {...record};
            if (record.id) {
                props.editTopicObjective(record.id, record, token);
                setNewRecord(_record)
                props.getAdminTopicObjectives(token);
                setProductDialog(true);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'OBJECTIVES UPDATED', life: 3000 });
            }
            else {
    ...
    const deleteProduct = () => {
        ...
        setDeleteProductDialog(false);
        setRecord(emptyRecord);
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Objective Deleted', life: 3000 });
    }
    ...
    const deleteSelectedProducts = () => {
        ...
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Objectives Deleted', life: 3000 });
    }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <Button label="OBJECTIVES" icon="pi pi-plus" className="p-button-success p-mr-2" />
            </React.Fragment>
        )
    }
    ...
    const header = (
        <div className="table-header">
            <h1 className="p-m-0">MANAGE OBJECTIVES</h1>
        ...
        <Paper className={classes.pageContent}>
            <div className="datatable-crud-demo">
                ...
                <Dialog visible={deleteProductsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteProductsDialogFooter} onHide={hideDeleteProductsDialog}>
                    <div className="confirmation-content">
                        <i className="pi pi-exclamation-triangle p-mr-3" style={{ fontSize: '2rem'}} />
                        {record && <span>Are you sure you want to delete the selected objectives?</span>}
                    </div>
                </Dialog>
            </div>
          </Paper>
