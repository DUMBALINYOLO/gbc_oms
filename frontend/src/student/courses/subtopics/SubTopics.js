...
const SubTopics = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          props.getAdminSubTopics(props.data.id, token);
      }
    ...
    const saveProduct = (e) => {
        ...
        if (record.id) {
            ...
            props.getAdminSubTopics(props.data.id, token);
            setProductDialog(true);
            toast.current.show({ severity: 'success', summary: 'Successful', detail: 'SUBTOPIC UPDATED', life: 3000 });
        }
        else {
            props.addSubTopic(_record, token)
            setNewRecord(_record)
            props.getAdminSubTopics(props.data.id, token);
            toast.current.show({ severity: 'success', summary: 'Successful', detail: 'SUBTOPIC CREATED', life: 3000 });
        }
        setProductDialog(false);
        setRecord(emptyRecord);
    }
    ...
    const deleteProduct = () => {
        ...
        setDeleteProductDialog(false);
        setRecord(emptyRecord);
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Subtopic Deleted', life: 3000 });
    }
    ...
    const deleteSelectedProducts = () => {
        ...
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Subtopics Deleted', life: 3000 });
    }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <Button label="SUBTOPICS" icon="pi pi-plus" className="p-button-success p-mr-2" />
            </React.Fragment>
        )
    }
            ...
            <div className="datatable-crud-demo">
                ...
                <div className="card">
                    ...
                    <DataTable
                        ...
                        currentPageReportTemplate="SHOWING {first} TO {last} OF {totalRecords} SUBTOPICS"
