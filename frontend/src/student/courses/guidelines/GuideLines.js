...
import { getAdminTopicGuidelines } from '../../../actions/courses';
...
const GuideLines = (props) => {
    ...
    useEffect(() => {
      if(!props.fetched) {
          props.getAdminTopicGuidelines(props.data.id, token);
      }
    ...
    const saveProduct = (e) => {
        ...
        if (record.name.trim()) {
            ...
            let _record = {...record};
            if (record.id) {
                props.editTopicGuideline(record.id, record, token);
                setNewRecord(_record)
                props.getAdminTopicGuidelines(props.data.id, token);
                setProductDialog(true);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'GUIDELINE UPDATED', life: 3000 });
            }
            else {
                props.addTopicGuideLine(_record, token)
                setNewRecord(_record)
                props.getAdminTopicGuidelines(props.data.id, token);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'GUIDELINE CREATED', life: 3000 });
            }
            setProductDialog(false);
            setRecord(emptyRecord);
        }
    ...
    const deleteProduct = () => {
        ...
        setDeleteProductDialog(false);
        setRecord(emptyRecord);
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Guideline Deleted', life: 3000 });
    }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <Button label="GUIDELINES" icon="pi pi-plus" className="p-button-success p-mr-2" />
            </React.Fragment>
        )
    }
            ...
            <div className="datatable-crud-demo">
                ...
                <div className="card">
                    ...
                    <DataTable
                        ...
                        currentPageReportTemplate="SHOWING {first} TO {last} OF {totalRecords} GUIDELINES"
