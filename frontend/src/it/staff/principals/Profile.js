...
import {getPrincipalProfile, editPrincipalProfile} from "../../../actions/people";
...
class Profile extends React.Component {
  ...
  componentDidMount() {
    if (this.props.token !== undefined && this.props.token !== null) {
      this.props.getPrincipalProfile(this.props.id, this.props.token);
    }
  }
  ...
  componentWillReceiveProps(newProps) {
    if (newProps.token !== this.props.token) {
      if (newProps.token !== undefined && newProps.token !== null) {
        this.props.getPrincipalProfile(this.props.id, newProps.token);
      }
    }
  }
  ...
  addOrEdit = (profile, resetForm) => {
    this.props.editPrincipalProfile(this.props.id, profile, this.props.token);
    resetForm();
...
const mapStateToProps = state => {
  return {
    profile: state.people.principalprofile,
    token: state.auth.token,
  };
};

const constDispatchToProps = dispatch => ({
  getPrincipalProfile: bindActionCreators(getPrincipalProfile, dispatch),
  editPrincipalProfile: bindActionCreators(editPrincipalProfile, dispatch),
