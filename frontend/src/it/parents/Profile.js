...
import {getParentProfile, editParentProfile} from "../../actions/people";
...
class Profile extends React.Component {
  ...
  componentDidMount() {
    if (this.props.token !== undefined && this.props.token !== null) {
      this.props.getParentProfile(this.props.id, this.props.token);
    }
  }
  ...
  componentWillReceiveProps(newProps) {
    if (newProps.token !== this.props.token) {
      if (newProps.token !== undefined && newProps.token !== null) {
        this.props.getParentProfile(this.props.id, newProps.token);
      }
    }
  }
  ...
  addOrEdit = (profile, resetForm) => {
    this.props.editParentProfile(this.props.id, profile, this.props.token);
    resetForm();
...
const mapStateToProps = state => {
  return {
    profile: state.people.parentprofile,
    token: state.auth.token,
  };
};

const constDispatchToProps = dispatch => ({
  getParentProfile: bindActionCreators(getParentProfile, dispatch),
  editParentProfile: bindActionCreators(editParentProfile, dispatch),
