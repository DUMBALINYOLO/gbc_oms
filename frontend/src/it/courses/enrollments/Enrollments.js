...
const Enrollments = (props) => {
    ...
    const saveProduct = (e) => {
        ...
        let _record = {...record};
        if (record.id) {
            props.editStudentCourseEnrollment(record.id, record, token);
            setNewRecord(_record)
            props.getStudentCourseEnrollments(props.data.id, token);
            setProductDialog(true);
            toast.current.show({ severity: 'success', summary: 'Successful', detail: 'ENROLLMENT UPDATED', life: 3000 });
        }
        else {
            props.addStudentCourseEnrollment(_record, token)
            setNewRecord(_record)
            props.getStudentCourseEnrollments(props.data.id, token);
    ...
    const header = (
        <div className="table-header">
            <h1 className="p-m-0">MANAGE ENROLLMENTS</h1>
...
export default connect(
  mapStateToProps,
  {getStudentCourseEnrollments, addStudentCourseEnrollment, editStudentCourseEnrollment, getStudentProfiles, getCourseEnrollmentStatusChoices} )
  (Enrollments);
