...
const Authors = (props) => {
    ...
    const saveProduct = (e) => {
        ...
        if (record.name.trim()) {
            ...
            let _record = {...record};
            if (record.id) {
                props.editAuthor(record.id, record, token);
                setNewRecord(_record)
                props.getAuthors(token);
                setProductDialog(true);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'AUTHOR UPDATED', life: 3000 });
            }
            else {
                props.addAuthor(_record, token)
                setNewRecord(_record)
                props.getAuthors(token);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'AUTHOR CREATED', life: 3000 });
            }
            setProductDialog(false);
            setRecord(emptyRecord);
        }
    ...
    const deleteProduct = () => {
        ...
        setDeleteProductDialog(false);
        setRecord(emptyRecord);
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Author Deleted', life: 3000 });
    }
    ...
    const deleteSelectedProducts = () => {
        ...
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Authors Deleted', life: 3000 });
    }
      ...
      <>
        <Paper className={classes.pageContent}>
            <div className="datatable-crud-demo">
                ...
                <Dialog visible={productDialog} style={{ width: '500px' }} header="AUTHOR FORM" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                  <Form>
                    <Grid container>
                      <Grid item xs={12}>
                        <Controls.Input
                ...
                <Dialog visible={deleteProductsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteProductsDialogFooter} onHide={hideDeleteProductsDialog}>
                    <div className="confirmation-content">
                        <i className="pi pi-exclamation-triangle p-mr-3" style={{ fontSize: '2rem'}} />
                        {record && <span>Are you sure you want to delete the selected authors?</span>}
                    </div>
                </Dialog>
            </div>
          </Paper>
