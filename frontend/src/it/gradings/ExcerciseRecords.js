...
const Excercises = (props) => {
    ...
    const saveProduct = (e) => {
        ...
        if (record.name.trim()) {
            ...
            let _record = {...record};
            if (record.id) {
                props.editExcerciseRecord(record.id, record, token);
                setNewRecord(_record)
                props.getExcerciseRecords(token);
                setProductDialog(true);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'EXERCISE RECORDS UPDATED', life: 3000 });
            }
            else {
                setNewRecord(_record)
                props.getExcerciseRecords(token);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'EXERCISE RECORDS CREATED', life: 3000 });
            }
            setProductDialog(false);
            setRecord(emptyRecord);
        }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <Button label="EXERCISE RECORDS" icon="pi pi-plus" className="p-button-success p-mr-2" />
            </React.Fragment>
        )
    }
                    ...
                    <Grid container>
                      <Grid item xs={12}>
                        <Controls.Input
                          ...
                          label="SCORE"
                          value={record.score}
                          onChange={(e) => onInputChange(e, 'score')}
                        />
                      </Grid>
                    </Grid>
                  </Form>
