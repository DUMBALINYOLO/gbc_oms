...
const AssignmentRecords = (props) => {
    ...
    const saveProduct = (e) => {
        ...
        if (record.name.trim()) {
            ...
            let _record = {...record};
            if (record.id) {
                props.editAssignmentRecord(record.id, record, token);
                setNewRecord(_record)
                props.getAssignmentRecords(token);
                setProductDialog(true);
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <Button label="ASSIGNMENT RECORDS" icon="pi pi-plus" className="p-button-warning p-mr-2" />
            </React.Fragment>
        )
    }
