...
const TeacherAttendances = (props) => {
    ...
    const saveProduct = (e) => {
        ...
        let _record = {...record};
        if (record.id) {
            props.editAdminAttendance(record.id, record, token);
            setNewRecord(_record)
            props.getAdminAttendances(token);
            setProductDialog(true);
            toast.current.show({ severity: 'success', summary: 'Successful', detail: 'ATTENDANCE UPDATED', life: 3000 });
        }
        else {
            props.addTeacherAttendance(_record, token)
            setNewRecord(_record)
            props.getAdminAttendances(token);
            toast.current.show({ severity: 'success', summary: 'Successful', detail: 'ATTENDANCE CREATED', life: 3000 });
        }
        setProductDialog(false);
        setRecord(emptyRecord);
    }
            ...
            <div className="datatable-crud-demo">
                ...
                <div className="card">
                    ...
                    <DataTable
                        ...
                        currentPageReportTemplate="SHOWING {first} TO {last} OF {totalRecords} ATTENDANCES"
...
export default connect(
  mapStateToProps,
  {getAdminAttendances, editAdminAttendance, addTeacherAttendance, getClasses, getTeacherProfiles} )
  (TeacherAttendances);
