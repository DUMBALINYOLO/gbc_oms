...
const Assignments = (props) => {
    ...
    const saveProduct = (e) => {
        ...
        if (record.name.trim()) {
            ...
            if (record.id) {
                ...
                props.getAdminStudentAssignments(token);
                setProductDialog(true);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'ASSIGNMENT UPDATED', life: 3000 });
            }
            else {
                props.addGrade(_record, token)
                setNewRecord(_record)
                props.getAdminStudentAssignments(token);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'ASSIGNMENT CREATED', life: 3000 });
            }
            setProductDialog(false);
            setRecord(emptyRecord);
        }
    ...
    const deleteProduct = () => {
        ...
        setDeleteProductDialog(false);
        setRecord(emptyRecord);
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Assignment Deleted', life: 3000 });
    }
    ...
    const deleteSelectedProducts = () => {
        ...
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Assignments Deleted', life: 3000 });
    }
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <Button label="CREATE ASSIGNMENT" icon="pi pi-plus" className="p-button-success p-mr-2" onClick={openNew} />
            </React.Fragment>
        )
    }
      ...
      <TeacherLayout>
        <Paper className={classes.pageContent}>
            <div className="datatable-crud-demo">
                ...
                <Dialog visible={productDialog} style={{ width: '500px' }} header="ASSIGNMENT FORM" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                  <Form>
                    <Grid container>
                      <Grid item xs={6}>
                        <Controls.Input
...
export default connect(
  mapStateToProps,
  {getClasses, getGeneralGradingTypeChoices, getTeacherProfiles, getSubjects, getAdminStudentAssignments, addGrade, editGrade} )
  (Assignments);
